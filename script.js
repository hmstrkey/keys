...
async function generateKeys(appToken, promoId, count) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        // Имитация генерации ключа на основе appToken и promoId
        const key = generateFakePromoCode(appToken, promoId);
        keys.push(key);
    }
    return keys;
}

function generateFakePromoCode(appToken, promoId) {
    // Создаем "реалистичный" промо-код, используя appToken и promoId
