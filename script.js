...
async function login(clientId) {
    if (DEBUG_MODE) {
        return `${APP_TOKEN}:deviceid:${generateDeviceId()}:${Date.now()}`;
    }
...
async function generateKey(clientToken) {
    try {
        ...
        // Логируем полученный ответ, чтобы увидеть весь промокод
        console.log('Generated promo code response:', data);

        return data.promoCode; // Убедитесь, что весь промокод возвращается
    } catch (error) {
    ...
    if (key) {
        ...
        if (USER_ID) {
            ...
            if (result.status === 'ok') {
                ...
                appendToDisplay('Ошибка при сохранении ключа.');
                return null;
            }
        } else {
            return key;
        ...
        appendToDisplay('Ошибка при генерации ключа.');
        return null;
    }
}
